...
NxWheel2::NxWheel2(NxActor* a, NxWheelDesc* wheelDesc) : actor(a)
{
	...
	if(!wsm)
	{
		NxMaterialDesc m;
		m.setToDefault();
		//m.flags |= NxMaterialFlag::NX_MF_DISABLE_FRICTION;
		wsm = scene->createMaterial(m);
	}

	NxWheelShapeDesc wheelShapeDesc;
	wheelShapeDesc.setToDefault();
	//Posicao
	wheelShapeDesc.localPose.t = wheelDesc->position;
	...
	wheelShapeDesc.lateralTireForceFunction.asymptoteSlip *= fat;	
	wheelShapeDesc.longitudalTireForceFunction.asymptoteSlip *= fat;	

	wheelShapeDesc.lateralTireForceFunction.asymptoteValue *= fat;	
	wheelShapeDesc.longitudalTireForceFunction.asymptoteValue *= fat;

	wheelShapeDesc.lateralTireForceFunction.extremumSlip *= fat;	
	wheelShapeDesc.longitudalTireForceFunction.extremumSlip *= fat;

	wheelShapeDesc.lateralTireForceFunction.extremumValue *= fat;	
	wheelShapeDesc.longitudalTireForceFunction.extremumValue *= fat;*/

	//wheelShapeDesc.density = 1000000.;
	//wheelShapeDesc.lateralTireForceFunction.setToDefault();
	//wheelShapeDesc.longitudalTireForceFunction.setToDefault();
	//wheelShapeDesc.setToDefault();
	wheelShape = static_cast<NxWheelShape *>(actor->createShape(wheelShapeDesc));

	wheelShape->setWheelFlags(wheelFlags);
	//wheelShape->setBrakeTorque(710.);
}
