...
Defender::Defender(QObject* p, Robot* r, int position, qreal speed)
	: Tactic(p,r),
	goto_(new Goto(this, r))//, kickTo_(new KickTo(this, r))//, getBall_(new GetBall(this, r, 1000))
{
	position_ = position; 
...
void Defender::step()
{
	...
	Line bolaGol(myGoal->x(), myGoal->y(), ball->x(), ball->y());

	//Nova versao
	Robot* Inimigo = enemyTeam->getClosestPlayerToBall();
	...
	Line ballGoalLine = Line(ball->x(),ball->y(),robot->goal()->x(),robot->goal()->y());
...
*/
	...
	Point ponto(myGoal->x() - (myGoal->x() - ball->x()) * raio / bolaGol.length(), - (myGoal->y() - ball->y()) * raio / bolaGol.length());
	...
	*/
	...
	Line IniBallLine = Line(ball->x(), ball->y(), IniX, IniY);
	//Line IniBallLine = Line(ball->x(), ball->y(), myGoal->x(), 0.0);
	...
	GoalLine = Line(myGoal->x(), myGoal->y(), stage->ball()->x(), stage->ball()->y());
