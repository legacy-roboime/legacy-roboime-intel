...
ActorPicking::ActorPicking(NxPhysicsSDK* physicsSDK, NxScene* scene)//, NxDistanceJoint* gMouseJoint, NxActor* gMouseSphere, NxReal gMouseDepth, NxActor* gSelectedActor)
	: actors(scene, physicsSDK), 
	joints(scene)
{
	this->physicsSDK = physicsSDK;
	this->scene = scene;
	this->gMouseJoint = NULL;
	this->gMouseSphere = NULL;
	this->gMouseDepth = NULL;
	this->gSelectedActor = NULL;
}

void ActorPicking::ViewProject(NxVec3 &v, int &xi, int &yi, float &depth)
{
...
void ActorPicking::ViewUnProject(int xi, int yi, float depth, NxVec3 &v)
{
//We cannot do picking easily on the xbox/PS3/WII anyway
#if defined(_XBOX) || defined (__CELLOS_LV2__) || defined(__PPCGEKKO__)
...
void ActorPicking::LetGoActor()
{
	if (gMouseJoint) 
		scene->releaseJoint(*gMouseJoint);
	gMouseJoint = NULL;
	if (gMouseSphere)
		scene->releaseActor(*gMouseSphere);
	gMouseSphere = NULL;
}

// ------------------------------------------------------------------------------------

bool ActorPicking::PickActor(int x, int y)
{
	...
	gMouseSphere = actors.CreateSphere(hit.worldImpact, 0.1f, 1.0f);
...
void ActorPicking::MoveActor(int x, int y)
{
	if (gMouseSphere == NULL) 
		return;
	NxVec3 pos;
