...
Actors::Actors(NxScene* scene, NxPhysicsSDK* physicsSDK)
{
	this->scene = scene;
...
//	convexShapeDesc.meshData = physicsSDK->createConvexMesh(convexDesc);
	InitCooking();
	...
	convexShapeDesc.meshData = physicsSDK->createConvexMesh(downstream);
...
NxActor* Actors::CreateConcaveObject(const NxVec3& pos, const NxVec3& boxDim, const NxReal density)
{
	...
	concaveShapeDesc.meshData = physicsSDK->createTriangleMesh(downstream);
	//
	...
	// by API "meshData->getReferenceCount() == 0". And, the release API is "physicsSDK->releaseTriangleMesh(*meshData);"
	//
...
//	centerShapeDesc.meshData = physicsSDK->createConvexMesh(centerDesc);
	InitCooking();
	UserStream upstream = UserStream("c:\\tmp.bin", false);
	UserStream downstream = UserStream("c:\\tmp.bin", true);
	bool status = CookConvexMesh(centerDesc, upstream);
	centerShapeDesc.meshData = physicsSDK->createConvexMesh(downstream);
...
//	leftEdgeShapeDesc.meshData = physicsSDK->createConvexMesh(leftEdgeDesc);
	InitCooking();
	...
	leftEdgeShapeDesc.meshData = physicsSDK->createConvexMesh(downstream2);
...
//	rightEdgeShapeDesc.meshData = physicsSDK->createConvexMesh(rightEdgeDesc);
	InitCooking();
	...
	rightEdgeShapeDesc.meshData = physicsSDK->createConvexMesh(downstream3);
